...
function VideoCard({
    ...
    return `
    <div class="card card-hover p-3 flex flex-col h-full cursor-pointer" tabindex="0" aria-label="Voir la vidéo ${title}" onclick="${onClick ? onClick : `Router.navigate('/public/videos/${id}')`}">
        <div class="aspect-w-16 aspect-h-9 bg-gray-200 rounded-lg overflow-hidden mb-2 flex items-center justify-center">
            <img src="${thumbnailUrl || 'https://placehold.co/320x180?text=Vidéo'}" alt="Miniature vidéo" class="object-cover w-full h-full">
        </div>
        <div class="flex-1 flex flex-col justify-between">
            ...
            <div class="flex items-center justify-between mt-2">
                ${showVotes ? `<span class="badge badge-info"><i class="fas fa-thumbs-up mr-1"></i>${votes ?? 0} vote${votes === 1 ? '' : 's'}</span>` : ''}
